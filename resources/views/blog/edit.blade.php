    ...
    <div class="max-w-5xl mx-auto space-y-6" x-data="blogForm()">
        ...
        <form action="{{ route('blogs.update', $blog) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {{-- RIGHT: Meta --}}
                <div class="space-y-6">
                    <x-form.select-search name="status" label="Status" :options="['draft' => 'Draft', 'published' => 'Published']" :value="$blog->status" />
                    ...
                    <x-form.select-search name="categories" label="Category" :options="$categories->pluck('name', 'id')->toArray()" :value="$blog->categories->pluck('id')->toArray()" multiple />
                    <x-form.select-search name="tags" label="Tags" :options="$tags->pluck('name', 'name')->toArray()" :value="$blog->tags->pluck('name')->toArray()" multiple />

                    <x-form.file name="featured_image" label="Featured Image"
                        :current="$blog->featured_image_url"
