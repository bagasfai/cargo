...
@section('title', 'Create Expedition Price')
...
@section('content')
    <div class="max-w-5xl mx-auto space-y-6">
        ...
        <div class="flex justify-between items-center">
            <x-ui.heading level="h1">Create Expedition Price</x-ui.heading>
        ...
        <form action="{{ route('expedition-prices.store') }}" method="POST" class="space-y-6"
              x-data="{
                  ...
                  villages: [],
                  async fetchCities() {
                      if (!this.province_id) { this.cities = []; this.city_id = ''; this.districts = []; this.district_id = ''; this.villages = []; return; }
                      const res = await fetch(`/api/provinces/${this.province_id}/cities`);
                      this.cities = await res.json();
                      this.city_id = ''; this.districts = []; this.district_id = ''; this.villages = [];
                  },
                  async fetchDistricts() {
                      if (!this.city_id) { this.districts = []; this.district_id = ''; this.villages = []; return; }
                      const res = await fetch(`/api/cities/${this.city_id}/districts`);
                      this.districts = await res.json();
                      this.district_id = ''; this.villages = [];
                  },
                  async fetchVillages() {
                      if (!this.district_id) { this.villages = []; return; }
                      const res = await fetch(`/api/districts/${this.district_id}/villages`);
                      this.villages = await res.json();
                  }
              }"
              x-init="async () => {
                  if (province_id) await fetchCities();
                  if (city_id) await fetchDistricts();
                  if (district_id) await fetchVillages();
              }">
            @csrf
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {{-- Is Active --}}
                <div>
                    <x-form.toggle name="is_active" label="Active" :checked="old('is_active', true)" />
                </div>
            </div>
